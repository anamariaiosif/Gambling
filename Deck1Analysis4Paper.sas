...
* STUDY        : 	COCOA                                                 			    	*;
*                                                                                          	*;
* PROGRAM      : 	Deck1Analysis4Paper.sas                                                 		*;
*                                                                                       	*;
...
*********************************************************************************************;
*********************************************************************************************;
footnote "GAMBLING\Iowa Gambling\Deck 1" ; 
...
model deck1/n = block group|age_c  age_c|fsiqc/ solution ddfm=sat;
*IQ QUARTILES ARE -7, -1 and 7;
*AGE QUARTILES 
*Median= -0.516667
*Q1= -2.683333
*Q3= 2.816667;
estimate "slope in ASD" age_c 1 group*age_c 1 0;
/*
lsmeans group/ at age_c=-2.683333   at fsiqc=-7 cl ilink diff=all or; 
lsmeans group/ at age_c=-2.683333  at fsiqc=-1 cl ilink diff=all or; 
lsmeans group/ at age_c=-2.683333  at fsiqc=7 cl ilink diff=all or; 
lsmeans group/ at age_c=-0.516667   at fsiqc=-7 cl ilink diff=all or; 
lsmeans group/ at age_c=-0.516667  at fsiqc=-1 cl ilink diff=all or; 
lsmeans group/ at age_c=-0.516667  at fsiqc=7 cl ilink diff=all or; 
lsmeans group/ at age_c=2.816667   at fsiqc=-7 cl ilink diff=all or; 
lsmeans group/ at age_c=2.816667  at fsiqc=-1 cl ilink diff=all or; 
lsmeans group/ at age_c=2.816667  at fsiqc=7 cl ilink diff=all or; 
*/
...
*********************************************************************************************;
* MODELS ADJUSTING FOR FLUID;
*********************************************************************************************;
proc glimmix data=long1 (where=(group="ASD")) maxopt=1000 empirical= mbn ;
title 'ASD ONLY; Logistic model, block categorical, with Fluid Cognition';
   class subid  group block(ref="1");
model deck1/n = block fluid_c  age_c
 / solution ddfm=sat;
random intercept/ subject=subid ;
random _residual_ / subject=subid ;
run;
proc glimmix data=long1 (where=(group="TYP")) maxopt=1000 empirical= mbn ;
title 'TD ONLY; Logistic model, block categorical, with Fluid Cognition';
   class subid  block(ref="1");
model deck1/n = block NIH_Fluid_c  age_c 
 / solution ddfm=sat;
random intercept/ subject=subid ;
random _residual_ / subject=subid ;
run;
...
*Q1= -2.683333
*Q3= 2.816667;
estimate "age OR when iq is average for TD" age_c 1 age_c*fsiqc 0 age_c*group 0 1/exp cl;
estimate "age OR when iq is q1 for TD" age_c 1 age_c*fsiqc -7 age_c*group 0 1/exp cl;
estimate "age OR when iq is median for TD" age_c 1 age_c*fsiqc -1 age_c*group 0 1/exp cl;
estimate "age OR when iq is q3 for TD" age_c 1 age_c*fsiqc 7 age_c*group 0 1/exp cl;
estimate "age OR when iq is average for AsD" age_c 1 age_c*fsiqc 0 age_c*group 1 0/exp cl;
estimate "age OR when iq is q1 for ASD" age_c 1 age_c*fsiqc -7 age_c*group 1 0/exp cl;
estimate "age OR when iq is median for ASD" age_c 1 age_c*fsiqc -1 age_c*group  1 0/exp cl;
estimate "age OR when iq is q3 for AsD" age_c 1 age_c*fsiqc 7 age_c*group 1 0/exp cl;


estimate "FSIQ OR when age is average for TD or ASD" fsiqc 1 age_c*fsiqc 0/exp cl;
estimate "FSIQ OR when age is q1 for TD or ASD" fsiqc 1 age_c*fsiqc -2.683333 /exp cl;
estimate "FSIQ OR when age is median for TD or ASD" fsiqc 1 age_c*fsiqc -0.516667/exp cl;
estimate "FSIQ OR when age is q3 for TD or ASD" fsiqc 1 age_c*fsiqc 2.816667/exp cl;
/*
*/
random intercept/ subject=subid ;
random _residual_ / subject=subid ;
...
proc glimmix data=longF maxopt=1000 empirical= mbn;
title ;
   class subid  group01(ref="0") block(ref="1") ;
model deck1/n = block group01|age_c  age_c|fsiqc / solution ddfm=kr;
   title ;
random intercept/ subject=subid ;
random _residual_ / subject=subid ;
   title ;
...
proc glimmix data=longFQ maxopt=1000 empirical= mbn;
title ;
   class subid  group01(ref="0") block(ref="1");
model deck1/n = block group01|age_c  age_c|fsiqc/ solution ddfm=kr;
   title ;
